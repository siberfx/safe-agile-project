    ...
    <div class="mb-8">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center space-x-4">
                {{-- Add New Role Button --}}
                <a href="{{ route('admin.access.roles.create') }}" class="bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors flex items-center space-x-2">
                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 group">
                            <td class="px-8 py-6 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-semibold text-lg">
                                        <i class="fa-solid fa-user-shield"></i>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900 group-hover:text-gray-700 transition-colors">{{ $role->name }}</div>
                            ...
                            <td class="px-8 py-6 whitespace-nowrap">
                                <div class="text-sm text-gray-600 group-hover:text-gray-700 transition-colors">{{ $role->created_at->format('M d, Y') }}</div>
                            </td>
                            <td class="px-8 py-6 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-4">
                                    <a href="{{ route('admin.access.roles.show', $role->id) }}" class="text-blue-500 hover:text-blue-700 transition-colors p-2 rounded-lg hover:bg-blue-50" title="View">
                                        <i class="fa-solid fa-eye text-lg"></i>
                                    </a>
                                    <a href="{{ route('admin.access.roles.edit', $role->id) }}" class="text-primary hover:text-primary/80 transition-colors p-2 rounded-lg hover:bg-primary/10" title="Edit">
                                        <i class="fa-solid fa-pen text-lg"></i>
                                    </a>
                                    @if(!in_array($role->name, ['super_admin', 'admin', 'user']))
                                        <button class="text-red-500 hover:text-red-700 transition-colors p-2 rounded-lg hover:bg-red-50" title="Delete" onclick="deleteRole({{ $role->id }})">
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center space-y-3">
                                    <i class="fa-solid fa-user-shield text-4xl text-gray-300"></i>
                                    <span class="text-lg font-medium">No roles found.</span>
                                    <span class="text-sm text-gray-400">Create your first role to get started.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        document.addEventListener('DOMContentLoaded', function() {
            try {
                ...
                if (dataRows.length === 0) {
                    console.log('No roles data found, skipping DataTable initialization');
                    return;
                }

                console.log(`Initializing DataTable with ${dataRows.length} role rows`);

                // DataTables 2.x - Modern and simple initialization
                const rolesTable = new DataTable('#rolesTable', {
                    responsive: true,
                    processing: true,
                    pageLength: 10,
                    ...
                    language: {
                        ...
                        emptyTable: "Geen rollen gevonden",
                        zeroRecords: "Geen overeenkomende rollen gevonden",
                        paginate: {
                            first: "Eerste",
                            previous: "Vorige",
                            next: "Volgende",
        ...
        function deleteRole(roleId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.delete(`/system/access/roles/${roleId}`)
                        .then(response => {
                            Swal.fire(
                                'Deleted!',
